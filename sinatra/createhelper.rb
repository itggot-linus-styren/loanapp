...
module Sinatra
    module CreateHelper
        ...
        def update_route(loanmgr, usermgr, params)
            ...
            if loanmgr.has_association(@type)
                ...
                if @user && usermgr.has_permission?(@user, :update)
                    ...
                    flash[:error] = "You don't have permission to update loanables."
                    has_error = true
                end                
            else
                flash[:error] = "The loanable type \"#{@type}\" does not exist!"
            ...
            unless has_error
                if loanmgr.is_loaned?(@type, @loanable)
                    flash[:error] = "#{@loanable.name.capitalize} is loaned and can't be updated."
                    has_error = true
                end
            end
        ...
        def create_or_update(loanable, params, name)
            ...
            loanable.find_or_initialize_by(:name => name).update!(params[:create])
        end
        ...
        def postupdate_route(loanmgr, params)
            form = params[:create]
            name = form[:name]
            type = session[:type]
            ...
            if has_error
                ...
                flash[:notify] = "The #{loanable_name} \"#{name}\" was successfully updated."
                redirect "/"
            end
        end
